...
class Main(object):
    ...
    def run(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM rent ORDER BY keyword ASC, itemtime DESC ,crawtime DESC')
            ...
            with file:
                file.write('''<html>
                    ...
                    <title>上海租房信息 | ''' + datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S') + '''</title>
